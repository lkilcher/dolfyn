...
from adcp import adcp_header,adcp_config,adcp_raw
...
time_offset=0 ### This causes the time information to be returned in python's datenum format.  See pylab's date2num and num2date functions for more information.

# time_offset=366 ### Uncommenting this line puts the data in "Matlab's datenum" format.
...
class ensemble(object):
    ...
    def clean_data(self,):
        for nm in ['u','v','w','err_vel']:
            dat=getattr(self,nm)
            dat[dat==-32.768]=np.NaN
...
class adcp_loader(object):
    ...
    def read_dat(self,id):
        ...
        function_map = {0    : (self.read_fixed,[]),   # 0000
                        ...
                        12288: (self.skip_Nbyte,[32]), # 3000 Fixed attitude data format for OS-ADCPs #### This is pretty idiotic - for OS-ADCPs (phase 2) they suddenly decided to code the number of bytes into the header ID word. And then they don't really document what they did! So, this is cruft of a high order, and although it works on the one example I have - caveat emptor....
                        }
        ## Call the correct function:
        if function_map.has_key(id):
    ...
    def read_nocode(self,id):
        #### There appear to be codes 0340-03FC to deal with. I am not going to decode them, but I am going to try to figure out how many bytes to skip.
        # Does he mean "3040-30FC", or is the code wrong?  Hopefully the former.
        if hex(id)[2:4]=='30':
            #### I want to count the number of 1s in the middle 4 bits of the 2nd two bytes.
            nflds=bin(id[3] & 60).count('1')+bin(id[4] & 60).count('1') # 60 is a 0b00111100 mask
    ...
    def read_fixed(self,):
        if hasattr(self,'configsize'):# and False:
            ...
            # The other option may be that if a problem is encountered, I could come back here and re-read the header.
    ...
    def read_var(self,):
        ...
        ens=self.ensemble
        k=ens.k
        self.vars_read+=['number','rtc','number','BIT','ssp','depth_m','heading_deg','pitch_deg','roll_deg','salinity','temperature_C','mpt_sec','heading_std','pitch_std','roll_std','adc']
    ...
    def read_vel(self,):
        ...
        self.vars_read+=var_nms
        k=ens.k
        shp=[4,self.cfg['n_cells']]
        vels=np.array(self.f.read_i16(4*self.cfg['n_cells'])).reshape(shp,order='F')*.001
        for idx,nm in enumerate(var_nms):
            getattr(ens,nm)[:,k]=vels[idx,:]
        self._nbyte=2+4*self.cfg['n_cells']*2

    def read_corr(self,):
        k=self.ensemble.k
        self.vars_read+=['corr']
        self.ensemble.corr[:,:,k]=np.array(self.f.read_ui8(4*self.cfg['n_cells'])).reshape((self.cfg['n_cells'],4))
        self._nbyte=2+4*self.cfg['n_cells']

    def read_echo(self,):
        k=self.ensemble.k
        self.vars_read+=['echo']
        self.ensemble.echo[:,:,k]=np.array(self.f.read_ui8(4*self.cfg['n_cells'])).reshape((self.cfg['n_cells'],4))
        self._nbyte=2+4*self.cfg['n_cells']

    def read_prcnt_gd(self,):
        self.vars_read+=['prcnt_gd']
        self.ensemble.prcnt_gd[:,:,self.ensemble.k]=np.array(self.f.read_ui8(4*self.cfg['n_cells'])).reshape((self.cfg['n_cells'],4))
        self._nbyte=2+4*self.cfg['n_cells']

    def read_status(self,):
        self.vars_read+=['status']
        self.ensemble.status[:,:,self.ensemble.k]=np.array(self.f.read_ui8(4*self.cfg['n_cells'])).reshape((self.cfg['n_cells'],4))
        ...
        if cfg['prog_ver']>=5.3:
            ...
            if cfg['name']=='wh-adcp':
                if cfg['prog_ver']>=16.20:
                    #### RDI documentation claims these extra bytes were added in v8.17, but they don't appear in my 8.33 data - conversation with Egil suggests they were added in 16.20
                    fd.seek(4,1)
                    self._nbyte+=4
    ...
    def read_vmdas(self,):
        ...
        fd=self.f
        #### The raw files produced by VMDAS contain a binary navigation data block.
        ...
        time=datetime.timedelta(milliseconds=(int(fd.read_ui32(1)[0]*10))) # This byte is in hundredths of seconds (10s of milliseconds).
        ens.stime[k]=date2num(date+time)+time_offset
        fd.seek(4,1) ### "PC clock offset from UTC"
        ens.slatitude[k]=fd.read_i32(1)*self._cfac
        ens.slatitude[k]=fd.read_ui32(1)*self._cfac
        ens.etime[k]=date2num(date+datetime.timedelta(milliseconds=int(fd.read_ui32(1)[0]*10)))+time_offset
        ...
        ens.ntime[k]=date2num(date+datetime.timedelta(milliseconds=int(fd.read_ui32(1)[0]*10)))+time_offset
        fd.seek(16,1)
        self._nbyte=2+76
    ...
    def read_winriver(self,nbt):
        ...
        if nbt==97:
            ...
            if l!=-1:
                ...
                self.ensemble.stime[k]=(int(strng[l+7:l+8])+(int(strng[l+9:l+10])+int(str[l+11:l+12])/60)/60)/24
                #MATLAB CODE: (sscanf(str(l+7:l+8),'%d')+(sscanf(str(l+9:l+10),'%d')+sscanf(str(l+11:l+12),'%d')/60)/60)/24;
    ...
    def init_data(self,):
        outd=adcp_raw()
        for nm in data_defs:
            outd.add_data(nm,np.empty(get_size(nm,self._nens,self.cfg['n_cells']),dtype=data_defs[nm][2]),group=data_defs[nm][1])
        self.outd=outd
    ...
    def search_buffer(self):
        ...
        search_cnt=0
        fd=self.f
        while search_cnt<self._search_num and ((id1[0]!=127 or id1[1]!=127 ) or not self.checkheader()):
            search_cnt+=1
            nextbyte=fd.read_ui8(1)
            id1[1]=id1[0];id1[0]=nextbyte
        if search_cnt==self._search_num:
            warnings.warn('Searched %d entries... Not a workhorse/broadband file or bad data encountered: -> %x' % (search_cnt,id1),ADCPWarning,)  # MAKE THIS AN ERROR/EXCEPTION?
        elif search_cnt>0:
            warnings.warn('Searched %d bytes to find next valid ensemble start' % search_cnt,ADCPWarning)
    ...
    def read_buffer(self,):
        ...
        while self.ensemble.k<self.ensemble.n_avg-1:
            ...
            offset=self.hdr.nbyte+2-byte_offset #### The 2 is for the checksum
...
if __name__=='__main__':
    ...
    #with adcp_loader("/home/lkilcher/data/pnnl/adcp/raw/From_Instrument/_RDI_000.001") as ldr:
    adcpd=ldr.load_data()
