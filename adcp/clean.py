...
def find_surface(apd,thresh=10,nfilt=1001):
    ...
    *thresh* specifies the threshold used in detecting the surface.
    (The amount that echo must increase by near the surface for it to be considered a surface hit)

    *nfilt* specifies the width of the nanmedianfilter applied to produce *d_range_filt*.
    
    """    
    # This finds the minimum of the echo profile:
    inds=np.argmin(apd.echo[:],axis=0)
    # This finds the first point that increases (away from the profiler) in the echo profile
    edf=np.diff(apd.echo[:].astype(np.int16),axis=0)
    inds2=np.max((edf<0)*np.arange(apd.shape[0]-1,dtype=np.uint8)[:,None,None],axis=0)+1
...
def nan_above_surface(adp,dfrac=0.9,vars=['u','v','w','err_vel','beam1vel','beam2vel','beam3vel','beam4vel','u_inst','v_inst','w_inst'],val=np.NaN):
    """
    NaN the values of the data that are above the surface (from the variable *d_range_filt*) in the *adp* object.

    *vars* specifies the values to NaN out.

    *dfrac* specifies the fraction of the depth range that is considered good (default 0.9).
...
def vel_exceeds_thresh(adcpo,thresh=10,source=None):
    """
    Find values of the velocity data that exceed a threshold value, and 

    Optional argument *source* may be:
       "beam"
       "earth"
       "inst"
    This specifies whether to use beam, earth or instrument velocities
    to find bad values.  All of these data sources (if they exist) are
    cleaned.
    """
    ...
    for dt in ['beam1vel','beam2vel','beam3vel','beam4vel','u','v','w','u_inst','v_inst','w_inst','err_vel']:
        if hasattr(adcpo,dt):
