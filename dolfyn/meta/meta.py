...
class unitsDict(dict):
    ...
    unitsDict instances support simple math operations (multiply, divide, power)

    The *key* of unitsDicts objects are the units, the values represent the power
    of that unit.  For example:
...
class varMeta(object):
    ...
    *units_style* specifies how to format the units.
    ...
    def __eq__(self,other):
        ...
        """
        if other.__class__ is varMeta and self.name==other.name and self._units==other._units:
            return True
        return False
    ...
    def __init__(self,name,units=None,dim_names=[],units_style=None,scale=0,vecnames={}):
        """
        """
    ...
    def _copy_rep(self,name=None):
        """
        A copy method for use in constructing new varMeta objects from a basic type.
        ...
        if name is None:
            ...
            name=self.name % name
        return varMeta(name,(self._units and self._units.copy()),list(self.dim_names),self._units_style,self.scale)

    def copy(self,name=None):
        """
        Return a copy of this varMeta object.

        Optional variable *name* may be used to create a copy of these units, with
        a new 'name'.
        """
