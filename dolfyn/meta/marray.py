...
In particular, the meta data tracked by this package is:
    ...
    name      - A string that is used in labels for the data in the
                array.  This is a math-mode LaTeX string (dollar
                signs are added).
    vecnames  - A list of strings that are different names for
                specific indices of an axis.
                e.g. for the 'xyz' dimension, you could define
                vecnames={'xyz'=['u','v','w']}
...
class marray(np.ndarray):
    ...
    def __getitem__(self, slc):
        ...
        if np.ndarray in out.__class__.__mro__ and (out.ndim > 0 and out.ndim < self.ndim):
            ...
            # meta.dim_names when this operation reduces the dimension
            # of the array.
    ...
    def __repr__(self,):
        if self.ndim == 0 or len(self) <= 1:
            return "marray({}, <{}[{}], {}{}:{}>)".format(self.__str__(),
                                                          self.meta.name,
                                                          self.units,
                                                          self.dtype,
                                                          self.shape,
