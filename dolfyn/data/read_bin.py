...
# It is mostly for development purposes, to simplify learning a data file's format.
# For final use, reading binary data files should minimize the number of calls to struct.unpack and file.read
# because many calls to these functions (i.e. using the code in this module) are slow.
import numpy as np
...
class checksum(object):
    ...
    # In that case, it *might* be nice to include functionality for nested checksums, but that seems a bit overkill at this point.
    ...
    def __call__(self,val,remove_val=False):
        ...
        *remove_val* specifies whether *val* should be removed from self._cs because it was already added to it.
        """
        if self._rem:
            self.error('rem')
...
class bin_reader(object):
    ...
    def __init__(self,fname,endian='<',checksum_size=None):
        ...
        *checksum_size* is in bytes, if it is None or False, this function does not perform checksums.
        """
        self.endian=endian
    ...
    def read(self,n,frmt):
        ...
        if n==1:
            ...
            return unpack(self.endian+frmt*n,val)
