...
class specobj(Dgroups):
    ...
    def _data_groups(self,):
       if not hasattr(self,'__data_groups__'):
           self.__data_groups__={'spec':oset(['Suu','Svv','Sww']),'_essential':oset(['freq'])}
    ...
    def trapz(self,inds=slice(None)):
        ...
        for nm in out.specvars:
            ...
            dims.pop(ax)
            out[nm]=np.trapz(out[nm][...,inds[nm]],out.freq[inds[nm]],axis=ax)
            out[nm].meta.dim_names=dims
        return out
    ...
    def __repr__(self,):
        return ("<specobj: "+len(self.specvars)*'%s, '+'(%s)>') % tuple(self.specvars+[self.freq.meta.name])
...
class cohereobj(specobj):
    ...
    def _data_groups(self,):
       if not hasattr(self,'__data_groups__'):
           self.__data_groups__={'spec':oset(['Coh_uu','Coh_vv','Coh_ww']),'_essential':oset(['freq'])}
       return self.__data_groups__

rad_per_cyc=ma.marray(2*np.pi,meta=ma.varMeta('',{'hz':-1,'s':-1},[]))
...
def ind_specobj(dat,inds=slice(None),typeobj=specobj,names=None,freqvar='omega',dflt_ax=0):
    """
    ...
    tup=False
    if names is None:
        names=out.specvars # This pulls the defaults from the chosen specobj.
        nms=names
    else:
