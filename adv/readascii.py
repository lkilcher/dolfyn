...
def count_lines(fd):
    ...
    val=(pe-p1)/(p2-p1)
    if val!=int(val):
        raise ValueError('Number of lines is not an integer: perhaps the bytes/line is not constant?')
    return int(val)
...
class header_reader(object):
    """
    This is incomplete.  It should eventually return an adv_config object...
    """
    ...
    def readline(self,):
        while True:
            ...
            while nxtln.startswith(' '):
                ...
                nxtln=self.fd.readline()
            self.fd.seek(pos,0)
            if ln.startswith('---') or ln.strip('\r\n') in ['Head configuration','Hardware configuration','User setup']:
                continue
            elif ln=='':
                return None
            return ln.strip('\r\n')
...
def read_sen(filename,advd):
    ...
    with open(filename,'r') as fe:
        ...
        for ind,ln in enumerate(fe):
            ...
            dt=ln.split()
            k=ind*sr
            advd.mpltime[k]=tbx.date2num(tbx.datetime(int(dt[2]),int(dt[0]),int(dt[1]),int(dt[3]),int(dt[4]),int(dt[5])))
...
def time_func(date,frmt):
    ...
    for frmt in frmts:
        try:
            return tbx.date2num(tbx.datetime.strptime(date,frmt))
        except:
            pass
    raise ValueError('No valid format in format specifier?')
    

def read_sontek_HorizonTab(filename,dat_map=['ensemble','_time:%m/%d/%Y %I:%M:%S %p;%m/%d/%Y','u','v','w','Amp1','Amp2','Amp3','corr1','corr2','corr3','SNR1','SNR2','SNR3',],dlm='\t'):
    """
...
def read_sontek_winadv(filename,dat_map=['mpltime',None,None,'u','v','w','corr1','corr2','corr3','SNR1','SNR2','SNR3','Amp1','Amp2','Amp3',None,None,None],dlm=';',skip_n_headlines=9):
    dat=read_dat(filename,dat_map,dlm,skip_n_headlines=skip_n_headlines)
    return dat

## def read_kr(filename,dat_map=['mpltime','burst','u','v','w','Amp1','Amp2','Amp3','SNR1','SNR2','SNR3'],frmt=['%19s %02d %03d %0.3f   %0.3f   %0.3f   %3d   %3d   %3d']):
##     pass

## def read_datstr(filename,map,strng):
##     with open(filename,'r') as fu:
##         k=0
##         nlines=int(count_lines(fu))
##         prog_bar=dio.progress_bar(nlines,40)
##         advd=adv.adv_raw(nlines,dat_map)
##         bd_time=0
##         for i,ln in enumerate(fu):
            
def read_dat(filename,dat_map=['burst','ensemble','u','v','w','Amp1','Amp2','Amp3','SNR1','SNR2','SNR3','corr1','corr2','corr3','pressure'],dlm=' ',skip_n_headlines=0,cls=adv.adv_raw,nlines=None):
    """
    Read ADV ascii file *filename*, and return a adv_raw object containing the data.
    ...
    *dat_map* specifies the variable names of the columns of the file.
    ...
    with open(filename,'r') as fu:
        ...
        advd.add_data('mpltime',np.empty(nlines,dtype=np.float64),'_essential')
        bd_time=0
        for i,ln in enumerate(fu):
            ...
            for ind,nm in enumerate(dat_map):
                ...
                if nm.__class__ in [str,unicode] and not nm.startswith('_'):
                ...
                elif nm.__class__ is time_func.__class__:# is 'nm' a function?
                    args=[dt[ind]]
                    itmp=1
                    ...
                    try:
                        ...
                        advd.mpltime[idx]=time_func(dt[ind],nm[6:15]) # See if it is only a date, presuming the format starts with date.
                    except ValueError:
                        bd_time+=1
                        break
            k+=1
...
def count_burst_lines(direc,sfx='.dat'):
    """
    Count the total number of lines in the files that end in *sfx* in directory *direc*.
