...
parser = argparse.ArgumentParser(description='Converts Nortek Vector .vec files from binary (.vec) format to Matlab(TM) (.mat).')
...
for fnm in args.files:
    ...
    # Save the data.
    dat.save_mat(outnm,groups=['main','orient','signal','#error','#env','#sys','#extra'])
